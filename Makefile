...
tables/dataset_stats.tsv:
	...
	cat dataset_formats.tsv | cut -f1,2,3,4,5,8,10 | awk -F '\t' '{ print $$3 "\t" $$2 "\t" $$7 "\thttps://globalbioticinteractions.org/?accordingTo=globi:" $$1 "\t" $$5 "\t" $$6 }' | sort | uniq | sort -nr > dataset_stats_no_header.tsv
	# add header
